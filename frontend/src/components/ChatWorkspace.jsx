...
export default function ChatWorkspace({ socket, userName = 'Yashwantika G.' }) {
  ...
  const [messages, setMessages] = useState({
    global: [
      { id: 1, user: 'Alex Rivera', text: 'Hey team, I set up the secure server bypass on Port 5000.', time: '13:51', avatarColor: 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' },
      { id: 2, user: 'Sarah Jenkins', text: 'Awesome! I am working on the whiteboard explain mode designs now.', time: '13:55', avatarColor: 'bg-pink-500/10 text-pink-400 border border-pink-500/20' }
    ],
    alex: [
      { id: 3, user: 'Alex Rivera', text: 'Hi Yashwantika, did you check the latency metrics?', time: '12:10', avatarColor: 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' }
    ],
    sarah: [
      { id: 4, user: 'Sarah Jenkins', text: 'Can we align on the new sidebar layout later today?', time: '10:04', avatarColor: 'bg-pink-500/10 text-pink-400 border border-pink-500/20' }
    ]
  })
    ...
    const handleIncomingMessage = async (encryptedPayload) => {
      try {
        ...
        const formattedMsg = {
          ...
          avatarColor: decMsg.avatarColor || 'bg-slate-700/10 text-slate-400 border border-slate-700/20'
        }
  ...
  return (
    <div className="flex h-full w-full bg-[#0b0f19] overflow-hidden select-text">
      
      {/* Left panel: Conversations List */}
      <aside className="w-80 border-r border-slate-800 bg-slate-900/10 flex flex-col flex-shrink-0">
            ...
            <button
              ...
              className={`w-full flex items-start gap-3 p-3.5 rounded-xl text-left transition-all ${
                activeConvId === conv.id 
                  ? 'bg-indigo-600/15 border border-indigo-500/20 text-white shadow-lg shadow-indigo-500/5' 
                  : 'border border-transparent text-slate-400 hover:text-slate-200 hover:bg-slate-900/50'
              }`}
            >
              <div className="w-9 h-9 rounded-xl bg-slate-800 border border-slate-700/50 flex items-center justify-center text-lg relative flex-shrink-0">
                {conv.avatar}
                {conv.status === 'Online' && (
              ...
              <div className="overflow-hidden">
                <div className="flex justify-between items-center">
                  <h4 className="text-xs font-bold text-slate-200 truncate">{conv.name}</h4>
                </div>
      ...
      <main className="flex-1 flex flex-col h-full bg-[#070b13]/40 relative">
        {/* Header */}
        <header className="h-14 border-b border-slate-800/80 px-6 flex items-center justify-between flex-shrink-0 select-none bg-slate-900/5">
          ...
          <button
            ...
            className="px-3.5 py-1.5 rounded-lg bg-slate-900 border border-slate-800 hover:bg-slate-850 hover:text-slate-200 text-slate-400 text-[10px] font-bold transition-all active:scale-95 flex items-center gap-1.5 shadow-md"
          >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {activeMessages.map((msg, index) => (
            ...
            >
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-extrabold text-[10px] select-none ${msg.avatarColor || 'bg-slate-800 text-slate-200'}`}>
              ...
              <div className="max-w-[70%] space-y-1">
                ...
                <div className={`px-4 py-2.5 rounded-2xl text-xs leading-relaxed border ${
                  msg.self 
                    ...
                    : 'bg-slate-900/60 text-slate-350 border-slate-800/80'
                }`}>
                  {msg.text}
                </div>
              </div>
        ...
        {/* Sticky Input Bar */}
        <form onSubmit={handleSend} className="p-4 border-t border-slate-800/85 bg-slate-900/10 flex items-center gap-3">
          <input
            type="text"
            value={inputText}
            onChange={(e) => setInputText(e.target.value)}
            placeholder={`Message ${activeConversation?.name}...`}
            className="flex-1 bg-slate-950/80 border border-slate-800 text-xs px-4 py-3 rounded-xl text-slate-200 focus:outline-none focus:border-indigo-500/80 placeholder-slate-600 shadow-inner"
          />
          <button
            type="submit"
            className="p-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 active:scale-95 text-white transition-all shadow-lg shadow-indigo-600/15"
          >
        ...
        {showFileModal && (
          <div className="fixed inset-0 bg-slate-950/85 backdrop-blur-sm flex items-center justify-center z-50 animate-[fadeIn_0.2s_ease-out]">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 w-full max-w-lg space-y-4 shadow-2xl relative">
              <button 
                type="button"
              ...
              <h3 className="text-xs font-bold text-slate-200 uppercase tracking-wider select-none">Securely Upload & Attach File</h3>
